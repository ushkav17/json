...
(function () {
  ...
  // ✅ Show both: last user question + start of bot reply
  function scrollToShowQuestionAndReply(userEl, botEl) {
    if (!userEl || !botEl) return;

    requestAnimationFrame(() => {
      const c = messagesEl.getBoundingClientRect();
      const u = userEl.getBoundingClientRect();

      // 1) Anchor: put user's last message near the top with padding
      const padTop = 16;
      messagesEl.scrollTop += (u.top - c.top) - padTop;

      // 2) Ensure bot reply top is visible too (without losing context)
      const c2 = messagesEl.getBoundingClientRect();
      const b2 = botEl.getBoundingClientRect();

      const minReplyTop = c2.top + 8;
      const maxReplyTop = c2.bottom - 40;

      if (b2.top < minReplyTop) {
        messagesEl.scrollTop -= (minReplyTop - b2.top);
      } else if (b2.top > maxReplyTop) {
        messagesEl.scrollTop += (b2.top - maxReplyTop);
      }

      // small nudge for nicer spacing
  ...
  async function startNewConversation() {
    ...
    try {
      ...
      if (data.data && Array.isArray(data.data)) {
        ...
        // default: show last message nicely
        ...
        botMsg.scrollIntoView({ block: "end", inline: "nearest" });
      }
    } catch (err) {
  ...
  async function sendMessage(message) {
    ...
    // show last question + typing
    scrollToShowQuestionAndReply(userMsg, typingMsg);
    ...
    try {
      ...
      // ✅ show last question + start of reply
      scrollToShowQuestionAndReply(userMsg, botMsg);
    } catch (err) {
      typingMsg.remove();
