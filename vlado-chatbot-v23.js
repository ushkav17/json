...
(function () {
  const styles = `
    ...
    :host .chat-container.position-left { right: auto; left: 20px; }
    :host .chat-container.open { display: flex; flex-direction: column; }
    ...
    :host .close-button:hover { opacity: 1; }
    :host .brand-header img { width: 32px; height: 32px; }
    :host .brand-header span { font-size: 18px; font-weight: 500; color: var(--chat--color-font); }
    ...
    :host .new-chat-btn:hover { transform: scale(1.02); }
    :host .message-icon { width: 20px; height: 20px; }
    :host .response-text { font-size: 14px; color: var(--chat--color-font); opacity: 0.7; margin: 0; }

    :host .chat-interface { display: none; flex-direction: column; height: 100%; }
    :host .chat-interface.active { display: flex; }
    ...
    :host .chat-message.bot.typing { font-style: normal; opacity: 1; padding: 10px 14px; }
    :host .typing-waves { display: inline-flex; align-items: flex-end; gap: 4px; height: 14px; }
    :host .typing-waves span {
    ...
    :host .typing-waves span:nth-child(2) { animation-delay: 0.15s; }
    :host .typing-waves span:nth-child(3) { animation-delay: 0.30s; }
    ...
    :host .chat-input textarea::placeholder { color: var(--chat--color-font); opacity: 0.6; }
    ...
    :host .chat-input button:hover { transform: scale(1.05); }
    ...
    :host .chat-toggle.position-left { right: auto; left: 20px; }
    :host .chat-toggle:hover { transform: scale(1.05); }
    :host .chat-toggle svg { width: 24px; height: 24px; fill: currentColor; }
    ...
    :host .chat-footer a:hover { opacity: 1; }

    @media only screen and (max-width: 600px),
      only screen and (orientation: landscape) and (max-height: 600px) {
      :host .chat-toggle { bottom: 75px; }
      :host .toggle-close { bottom: 150px; }
    }
  `;
  ...
  toggleButton.setAttribute("aria-label", "Przycisk otwierający NOYO chat");
  ...
  function renderBotResponse(mdText) {
    const html = window.marked
      ? marked.parse(mdText)
      : String(mdText ?? "").replace(/[<>&]/g, (c) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" }[c]));
    return window.DOMPurify ? DOMPurify.sanitize(html) : html;
  }

  /**
   * ✅ Keep BOTH visible:
   * - the last user message (at least some part)
   * - the start (top) of the bot reply / typing indicator
   */
  function scrollToShowUserAndBotStart(userEl, botEl) {
    if (!messagesEl || !userEl || !botEl) return;

    requestAnimationFrame(() => {
      const containerTop = messagesEl.scrollTop;
      const H = messagesEl.clientHeight;

      const userTop = userEl.offsetTop;
      const userBottom = userTop + userEl.offsetHeight;

      const botTop = botEl.offsetTop;

      // Prefer: bot start near top with a little padding
      const topPad = 8;
      let desired = Math.max(0, botTop - topPad);

      // Ensure some part of the user message stays visible
      // If user bottom would be above viewport, pull scroll up
      const minUserVisiblePx = 24;
      if (userBottom < desired + minUserVisiblePx) {
        desired = Math.max(0, userBottom - minUserVisiblePx);
      }

      // Also ensure bot start isn't pushed below the fold
      const botStartInView = botTop >= desired && botTop <= desired + (H - 24);
      if (!botStartInView) {
        // clamp so botTop ends up inside viewport
        desired = Math.max(0, botTop - (H - 24));
      }

      // Smooth-ish scrolling
      messagesEl.scrollTo({ top: desired, behavior: "smooth" });
    });
  }
    ...
    try {
      ...
      if (data.data && Array.isArray(data.data)) {
        data.data.forEach((item) => {
          ...
          else msgDiv.innerHTML = renderBotResponse(text);
          messagesEl.appendChild(msgDiv);
        });

        // show last user + last bot start if present
        const items = Array.from(messagesEl.children);
        const lastUser = [...items].reverse().find((el) => el.classList.contains("user"));
        const lastBot = [...items].reverse().find((el) => el.classList.contains("bot"));
        if (lastUser && lastBot && lastBot !== lastUser) scrollToShowUserAndBotStart(lastUser, lastBot);
        ...
        // keep welcome visible + bot start
        scrollToShowUserAndBotStart(welcomeMsg, botMsg);
      }
    } catch (err) {
  ...
  async function sendMessage(message) {
    ...
    typingMsg.innerHTML = `
      <div class="typing-waves" aria-label="Bot is typing" role="status">
        <span></span><span></span><span></span>
      </div>
    `;
    messagesEl.appendChild(typingMsg);

    // ✅ keep last user msg visible AND typing start visible
    scrollToShowUserAndBotStart(userMsg, typingMsg);
    ...
    try {
      ...
      // ✅ keep last user msg visible AND show START of bot reply
      scrollToShowUserAndBotStart(userMsg, botMsg);
    } catch (err) {
      typingMsg.remove();
