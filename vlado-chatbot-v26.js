...
(function () {
  ...
  // Load Geist Sans font (global so Shadow CSS font-family works)
  ...
  // ✅ Keep your original behavior, but allow a small upward offset
  ...
  // ✅ New: scroll to message top, then nudge UP so previous (user) message stays visible
  function scrollToMessageTopWithOffset(el, offsetPx) {
    if (!el) return;
    ...
    requestAnimationFrame(() => {
      ...
      // nudge after browser applies scrollIntoView
      requestAnimationFrame(() => {
        const maxScroll = Math.max(0, container.scrollHeight - container.clientHeight);
        const next = Math.min(Math.max(0, container.scrollTop - (offsetPx || 0)), maxScroll);
  ...
  async function startNewConversation() {
    ...
    try {
      ...
      if (data.data && Array.isArray(data.data)) {
        ...
        scrollToMessageTop(messagesEl.lastElementChild);
      } else {
  ...
  async function sendMessage(message) {
    ...
    // ✅ show typing, but nudge up a bit so user message remains visible
    scrollToMessageTopWithOffset(typingMsg, 40);
    ...
    try {
      ...
      // ✅ show start of reply BUT keep user question visible (nudge up)
      scrollToMessageTopWithOffset(botMsg, 70);
    } catch (err) {
      typingMsg.remove();
