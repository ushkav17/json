...
(function() {
    ...
    /* Updated position: above the toggle icon on the right side */
    :host .toggle-close {
      position: fixed;
      bottom: 90px;
      right: 7px;
    ...
    // Build chat elements inside Shadow DOM
    ...
    // Create a separate close button for the toggle icon
    const closeToggleBtn = document.createElement('button');
    closeToggleBtn.className = 'toggle-close';
    closeToggleBtn.textContent = '×';

    // Append chat container and, if not hidden by cookie, the toggle controls
    ...
    async function startNewConversation() {
        ...
        welcomeMsg.innerHTML = renderBotResponse(config.branding.welcomeText || 'Welcome!');
    ...
    // Hide the toggle icon entirely when the close-toggle button is clicked, and set a session cookie
